...
func GetClasses(level int) (cls []string) {
	for _, p := range newPatterns(level).db {
		cls = append(cls, p.name...)
	}
...
func (p *pattern) ToString(f bool) string {
	if f {
		return p.name[0]
	} else {
...
func (ps *patterns) Search(s string) *pattern {
	p := new(pattern)

	for _, v := range ps.db {
		if v.re.MatchString(s) {
			p.name = append(p.name, v.name...)
		}
	}

	if len(p.name) == 0 {
		p.name = append(p.name, "String")
	}

	return p
}

func newPatterns(level int) *patterns {
	str := &patterns{level: level}

	if level > 0 {
		str.db = append(str.db, patternsBasic()...)
	}

	if level > 1 {
		str.db = append(str.db, patternsMoney()...)
	}

	if level > 2 {
		str.db = append(str.db, patternsHashes()...)
	}

	return str
}

func patternsBasic() []pattern {
	return []pattern{
		{
...
func patternsHashes() []pattern {
	return []pattern{
		...
		{
			regexp.MustCompile("^\\$(krb5pa|mskrb5)\\$([0-9]{2})?\\$.+\\$[a-f0-9]+"),
			[]string{
		...
		{
			regexp.MustCompile("^SCRYPT:[0-9]+:[0-9]:[0-9]:[a-z0-9:/+=]+"),
			[]string{
				"scrypt",
			},
		},
		...
		{
			regexp.MustCompile("^\\$siemens-s7\\$[0-9]\\$[a-f0-9]{40}\\$[a-f0-9]{40}"),
			[]string{
				"Siemens-S7",
			},
		},
